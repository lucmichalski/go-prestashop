...
const (
	baseTemplate = `package efffiliation
...
const packageTemplate = `package effiliation
...
var ImportCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// https://apiv2.effiliation.com/apiv2/productfeeds.xml?key=APRLNEEAjmVnNppd7BWagymlOXplZnke&filter=mines
...
func init() {
	...
	ImportCmd.Flags().StringVarP(&proxyURLStr, "proxy", "x", "socks5://localhost:5566", "tor haproxy url")
	ImportCmd.Flags().BoolVarP(&psMarketplace, "ps-market", "", false, "create wk-marketplace users and catalogs per feed.")
...
func csv2ps(db *gorm.DB, fp, fo string, columns []string, separator string, cmp *CatalogMap) (*CatalogMap, int, error) {
	...
	for _, value := range rows {
		...
		go func(v map[string]string) error {
			...
			if err != nil {
				if strings.Contains(err.Error(), "bad content length") || strings.Contains(err.Error(), "Error") {
