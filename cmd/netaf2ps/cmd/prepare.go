...
func csv2ps(db *gorm.DB, fp, fo string, columns []string, separator string, cmp *CatalogMap) (*CatalogMap, int, error) {
	...
	headers := []string{"id_seller", "name", "reference", "category_id", "default_category", "short_description", "description", "price", "tax_id", "quantity", "image_ref", "feature_name", "feature_values", "minimum_garantie", "quantite_unitaire", "format_unitaire", "prix_total", "unite_total", "poids_piece", "ean13", "sku", "mpn", "id_product", "id_image"}
	...
	for _, value := range rows {
		...
		go func(v map[string]string) error {
			...
			row = append(row, fmt.Sprintf("%d", cmp.SellerId))
			...
			localImg, imageId, err := downloadImageToPs(imageRef, productId)
			...
			row = append(row, fmt.Sprintf("%d", productId))
			row = append(row, fmt.Sprintf("%d", imageId))
...
func downloadImageToPs(remoteUrl string, productId int) (string, int, error) {
	...
	return localImg, int(imageId), nil
